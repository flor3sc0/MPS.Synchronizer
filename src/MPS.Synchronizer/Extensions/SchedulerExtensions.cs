...
using MPS.Synchronizer.Application.SynchronizationJobs.Adverts;
...
public static class SchedulerExtensions
{
        ...
        services.UseScheduler(scheduler =>
            {
                foreach (var le in legalEntityOptions)
                {
                    ...
                    //AddStatisticsSchedulers(scheduler, le);
                    AddAdvertsSchedulers(scheduler, le);
                }
            })
    ...
    private static void AddStatisticsSchedulers(IScheduler scheduler, LegalEntityOptions le)
    {
    ...
    private static void AddAdvertsSchedulers(IScheduler scheduler, LegalEntityOptions le)
    {
        //scheduler.OnWorker("StatisticsSimple");

        //scheduler.ScheduleWithParams<AdvertsCampaignsSyncJob>(le)
        //    .DailyAt(le.Statistics.StatisticsIncomesSyncJobHour, le.Statistics.StatisticsIncomesSyncJobMinute)
        //    .Zoned(TimeZoneInfo.Local)
        //    .RunOnceAtStart()
        //    .PreventOverlapping($"{nameof(AdvertsCampaignsSyncJob)}_{le.Name}");

        scheduler.ScheduleWithParams<AdvertsCampaignSpendingHistoryJob>(le)
            .DailyAt(le.Statistics.StatisticsIncomesSyncJobHour, le.Statistics.StatisticsIncomesSyncJobMinute)
            .Zoned(TimeZoneInfo.Local)
            .RunOnceAtStart()
            .PreventOverlapping($"{nameof(AdvertsCampaignSpendingHistoryJob)}_{le.Name}");
    }
}